...
- **Authority order when they conflict**:
  ...
  4. Personal taste
- **Soft caps**: this document does not replace `task budget-status`.
...
`task test-race` (or `go test -race`) must be clean. Race detector
...
sub-test, so `task test-fn FN=TestValidate/missing_repo` works.
...
`task test-race`. Always. CI runs it. Failures are real, not flake;
see §11.
...
2. **`internal/config.Step` universal fields > 40** → flag (today's
   count is 36; check `task budget-status`).
...
### Use `task budget-status` before structural work
...
Documented in `CLAUDE.md`, regenerated by `task arch-snapshot`. The
...
```
task lint-pkg PKG=internal/actions/git_clone
task lint-new                            # only changed lines
task lint-fix                            # auto-fix what's auto-fixable
```

`task lint-fix` is safe. Run it before sending a PR.
...
- [ ] `task check-pkg PKG=<the package you touched>` (build + test
      with race + lint, sub-second for most edits).
- [ ] `task test-race` if the change crosses packages.
- [ ] `task lint-new` clean.
- [ ] `task budget-status` if you touched a handler, the executor,
...
schema is regenerated by `task schema-generate`.
...
```
task check-pkg PKG=internal/actions/os_hostname
task schema-generate     # if you added a new field to Step
task budget-status       # check Step field count
```
