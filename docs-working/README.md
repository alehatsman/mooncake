...
- 🟡 [#27](https://github.com/alehatsman/mooncake/issues/27) `artifact.capture` silently drops `capture_content`/checksum/size fields ([analysis](analysis/issue-27-artifact-capture-content-checksums-empty.md)) — schema/handler accept these fields but the upstream `FileOperationData` events don't carry the values, so they're empty + stripped by `omitempty`. Handler code admits the gap. Hurts the action's main use case (LLM agent loops with diff verification).
- 🟡 [#28](https://github.com/alehatsman/mooncake/issues/28) `mooncake schema validate --schema X` always reports out-of-date for freshly-generated X ([analysis](analysis/issue-28-schema-validate-always-out-of-date.md)) — `writer.go` writes indented JSON, `validateSchemaAction` compares against compact `MarshalJSON()`. Byte-compare guaranteed to fail. CI staleness checks for `internal/config/schema.json` are broken. Includes secondary docgen finding: platform-matrix table has malformed `||` separator row.
- 🟡 [#29](https://github.com/alehatsman/mooncake/issues/29) `mooncake runs apply/get/follow` exit 0 on errors (silent-success family) ([analysis](analysis/issue-29-runs-cli-silent-success-family.md)) — four exit-code bugs: (A) `apply` exits 0 when run fails before SSE stream starts (bad YAML / missing config), record.json shows failed; (B) `get`/`follow` exit 0 on unknown/malformed run id and on missing positional; (C) `--help` doesn't show required `<run_id>` positional; (D) `list --format text` silently outputs JSON. CI green when deploy fails.
