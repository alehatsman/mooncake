...
1. **The planner call is fully buffered.** `ClaudeCLIClient.GeneratePlan`
   (`internal/agent/llm/claude_cli_client.go:54`) runs the CLI with
...
2. **No event publisher exists during planning.** The per-step NDJSON lines are
   ...
   (`internal/agent/loop.go:~563`, mirrored in `run.go:105`). `plan.loaded`
   itself is emitted by the **executor** (`internal/executor/executor.go:1362`),
   not the agent loop. So during `RunLoop`'s `client.GeneratePlan(...)` call
...
- **Provider changes:** none. Pure agent-loop change.
...
- **Effort:** small. **Risk:** minimal. Note: `plan.loaded` is emitted by the
  executor, so this addition lives in the executor, not the agent.
...
1. **A planning-phase publisher/sink.** Hoist a publisher to `RunLoop` scope (or
   ...
   `cmd/kernel/agent.go`) so events emitted around/within `GeneratePlan` reach
...
2. **A streaming entry point (Option B only).** Extend the `Client` interface
   (`internal/agent/llm/client.go:10`). Two viable shapes:
...
- The terminal `agent.completed` contract is unchanged.
