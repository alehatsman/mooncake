// Command agent-evals runs the mooncake agent evaluation harness.
//
...
// internal/agent/llm.NewClient().GeneratePlan, then runs the goal's
// assertions against the returned plan YAML. Reports pass/fail per
// goal plus a summary.
//
// Gated on MOONCAKE_AGENT_EVAL=1 so accidental invocations do not
...
// HEAD~:docs-working/streams/agent/specs/spec-67-mooncake-agent.md)
// and testing-next/agent-evals/README.md.
package main
...
func main() {
	var (
		goalsDir = flag.String("goals", "testing-next/agent-evals/goals", "directory of goal YAML files")
		snapsDir = flag.String("snapshots", "testing-next/agent-evals/snapshots", "directory of snapshot JSON files")
	...
	if !*dryRun && os.Getenv("MOONCAKE_AGENT_EVAL") != "1" {
		fmt.Fprintln(os.Stderr, "refusing to run: set MOONCAKE_AGENT_EVAL=1 to opt in, or pass -dry-run to validate the harness without an LLM call")
		os.Exit(2)
	}
...
func fail(format string, args ...interface{}) {
	fmt.Fprintf(os.Stderr, "agent-evals: "+format+"\n", args...)
	os.Exit(1)
}
