# Mooncake pilot eval harness

Eval harness for the `mooncake pilot` (née `mooncake agent`) loop. See
...
```
MOONCAKE_PILOT_EVAL=1 go run ./testing-next/pilot-evals/
```
...
```
MOONCAKE_PILOT_EVAL=1 go run ./testing-next/pilot-evals/ -only 001-create-service-and-verify
```
...
├── run.go         # the runner (gated on MOONCAKE_PILOT_EVAL=1)
└── README.md      # this file
```
...
`.github/workflows/pilot-evals.yml.disabled` runs the harness on PRs
labeled `needs-pilot-eval`. The workflow is currently `.disabled` to
