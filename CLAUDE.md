...
- ✅ **Use a git worktree for implementation work** — any code change, spec implementation, or multi-PR feature must happen in a worktree, not on the active branch in the main checkout. Create one with `git worktree add ../mooncake-<branch> -b worktree-<branch>` and `cd` there before editing. When spawning sub-agents via the Agent tool for implementation, pass `isolation: "worktree"`. Doc-only edits (`docs-working/`, `internal/*/README.md`, README tweaks), analysis writeups, and read-only investigation can stay on the current branch. Note: `dist/docs/` is fully generated by `mooncake task docs-generate` — never hand-edit; touch the source (action metadata, package READMEs, CLI definitions) and regenerate.
