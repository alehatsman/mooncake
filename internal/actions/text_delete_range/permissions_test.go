package text_delete_range

import (
	"testing"

	"github.com/alehatsman/mooncake/internal/actions"
	"github.com/alehatsman/mooncake/internal/config"
)

// TestPermissions_TableDriven exercises text.delete_range's
// Permitter contract. Like its siblings, only Path drives Sudo —
// the anchor pair is informational.
func TestPermissions_TableDriven(t *testing.T) {
	h := &Handler{}
	tests := []struct {
		name      string
		path      string
		wantSudo  bool
		wantWrite []string
	}{
		{"system /etc", "/etc/security/limits.conf", true, []string{"/etc/security/limits.conf"}},
		{"system /opt", "/opt/myapp/config.ini", true, []string{"/opt/myapp/config.ini"}},
		{"user", "/home/user/scratch", false, []string{"/home/user/scratch"}},
		{"templated", "{{ etc_dir }}/foo", false, []string{"{{ etc_dir }}/foo"}},
		{"empty", "", false, nil},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			step := &config.Step{TextDeleteRange: &config.FileDeleteRange{
				Path:        tt.path,
				StartAnchor: "BEGIN",
				EndAnchor:   "END",
			}}
			perms := h.Permissions(step)
			if perms.Sudo != tt.wantSudo {
				t.Errorf("Sudo = %v, want %v", perms.Sudo, tt.wantSudo)
			}
			if perms.Network {
				t.Error("Network must be false for text.delete_range")
			}
			if !slicesEqual(perms.FilesystemWrite, tt.wantWrite) {
				t.Errorf("FilesystemWrite = %v, want %v", perms.FilesystemWrite, tt.wantWrite)
			}
		})
	}
}

func TestPermissions_NilHandled(t *testing.T) {
	h := &Handler{}
	if got := h.Permissions(nil); got.Sudo || len(got.FilesystemWrite) != 0 {
		t.Errorf("Permissions(nil) = %+v, want zero", got)
	}
	if got := h.Permissions(&config.Step{}); got.Sudo || len(got.FilesystemWrite) != 0 {
		t.Errorf("Permissions(empty step) = %+v, want zero", got)
	}
}

func TestPermissions_RegisteredAsPermitter(t *testing.T) {
	if !actions.IsPermitter(&Handler{}) {
		t.Error("*Handler should satisfy actions.Permitter")
	}
}

func slicesEqual(a, b []string) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
		if a[i] != b[i] {
			return false
		}
	}
	return true
}
