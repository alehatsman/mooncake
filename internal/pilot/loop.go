...
func RunLoop(opts RunOptions) (*LoopResult, error) {
	...
	for i := 1; i <= opts.MaxIterations; i++ {
		...
		outcome, err := applyPlanIteration(wrappedBytes, opts.RepoRoot, log, gate, opts.OutputFormat)
		if err != nil {
			return nil, err
		}
...
func applyPlanIteration(wrappedBytes []byte, repoRoot string, log logger.Logger, gate planGate, outputFormat string) (iterationOutcome, error) {
