...
`)
	...
	if !wrapper.AllowIrreversible {
		t.Error("wrapper.AllowIrreversible = false, want true (otherwise plan-time check rejects irreversible steps in pilot plans)")
	}
...
// DoD headline: a pilot plan whose step 3 fails must leave the system
// byte-identical to its pre-pilot state.
