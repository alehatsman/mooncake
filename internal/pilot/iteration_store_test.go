...
func TestCollectUntrackedFile(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "pilot.txt"), []byte("pilot was here\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if want := []string{"pilot.txt"}; !reflect.DeepEqual(files, want) {
...
// TestCollectExcludesPilotInternals confirms pilot's own transient state —
...
// plan did.
func TestCollectExcludesPilotInternals(t *testing.T) {
	dir := initRepoWithBaseCommit(t)

	if err := os.MkdirAll(filepath.Join(dir, ".mooncake/pilot/iterations"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, ".mooncake/pilot/iterations/00001.json"), []byte("{}"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if want := []string{"artifact.txt"}; !reflect.DeepEqual(files, want) {
		t.Errorf("changed files = %v, want %v (pilot internals must be excluded)", files, want)
	}
