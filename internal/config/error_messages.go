...
func formatOneOfError(err *jsonschema.ValidationError) string {
	...
	if hasRequiredFailure && !hasNotFailure {
		return "Step has no action. Each step must have exactly ONE of: shell, template, file, copy, service, assert, include, include_vars, or vars"
	}

	// If we have "not" failures, it means multiple actions are present
	if hasNotFailure {
		return "Step has multiple actions. Only ONE action is allowed per step. Choose either: shell, template, file, copy, service, assert, include, include_vars, or vars"
	}

	// Generic fallback
	return "Step must have exactly one action (shell, template, file, copy, service, assert, include, include_vars, or vars)"
}
