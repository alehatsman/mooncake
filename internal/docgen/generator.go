...
// stamp writes the "this file is generated" marker. Deterministic — no
// timestamp — so re-running docs-generate produces byte-identical output
// when the inputs (action metadata, schema, READMEs) haven't changed.
// That keeps `git status` quiet after a regen and lets docs-check use a
// plain recursive diff instead of stripping volatile lines.
//
// Version is included because it's also deterministic (set by the
// release build) and useful for spotting which binary produced a given
// committed copy.
func (g *Generator) stamp(w io.Writer) {
	write(w, "<!-- Generated by mooncake docs generate; version %s -->\n", g.Version)
}

// writeMetadataHeader writes the standard generation marker at the top
// of a single-section page, followed by any caller-supplied
// `<!-- key: value -->` lines and a blank line separator.
