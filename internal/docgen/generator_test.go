...
func TestGeneratePlatformMatrix(t *testing.T) {
	...
	// Check for the deterministic generation stamp. The footer is a
	// single line including version; no timestamp (intentional, to keep
	// regenerated output byte-identical when inputs are unchanged).
	if !strings.Contains(output, "<!-- Generated by mooncake docs generate; version test") {
		t.Error("missing generation stamp with version")
	}
