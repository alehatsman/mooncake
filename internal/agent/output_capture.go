...
// agentStdoutTailBytes caps the captured stdout fed back into the next
...
// HEAD.
const agentStdoutTailBytes = 4096

// agentStepSummaryLineBytes caps a single per-step summary line. Most
...
// step plan.
const agentStepSummaryLineBytes = 240

// agentStepSummariesMax bounds the per-iteration summary slice so a
...
const agentStepSummariesMax = 30

// stdoutCapture is a agent-internal events.Subscriber that does TWO
...
//     carries it forward (loop-termination half of the agent output-
...
// goroutine pool, so OnEvent runs on a non-agent goroutine. All
...
type stdoutCapture struct {
	...
	// field). Capped at agentStepSummariesMax; extra completions are
	// dropped with a single "... N more" sentinel appended.
	stepSummaries []string
	// stepSummariesDropped counts step.completed events that arrived
	// after agentStepSummariesMax was reached, so Summaries() can
...
func (c *stdoutCapture) OnEvent(event events.Event) {
	...
	case events.EventStepFailed:
		...
		c.failedStep = &FailedStepInfo{
			...
			Stderr:   truncateTail(data.Stderr, agentStdoutTailBytes),
...
func (c *stdoutCapture) handleCompleted(data events.StepCompletedData) {
	...
	c.mu.Lock()
	c.lastStdout = truncateTail(captured, agentStdoutTailBytes)
	c.mu.Unlock()
}

// recordSummary appends a one-line summary for the just-completed step
// to c.stepSummaries, bounded by agentStepSummariesMax. Designed to be
...
func (c *stdoutCapture) recordSummary(info startedInfo, data events.StepCompletedData) {
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	if len(c.stepSummaries) >= agentStepSummariesMax {
...
// in-memory maps that the surrounding agent iteration drops.
...
// truncated to agentStepSummaryLineBytes; the slice is capped at
// agentStepSummariesMax with a trailing "... N more steps omitted"
...
//
// The line is then clamped to agentStepSummaryLineBytes — handler
...
func summarizeStep(action, name string, changed bool, result map[string]interface{}) string {
	...
	line := b.String()
	if len(line) > agentStepSummaryLineBytes {
		const ellipsis = "..."
		line = line[:agentStepSummaryLineBytes-len(ellipsis)] + ellipsis
	}
...
// the executor package); duplicated here so agent doesn't reach into
