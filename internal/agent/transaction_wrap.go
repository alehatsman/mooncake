...
// transactionWrapName is the Name field set on agent's implicit
// transaction wrapper. Surfaced in event streams and the audit trail so
// operators can tell which step boundary belongs to agent vs. their
// own authored steps.
const transactionWrapName = "agent apply"

// wrappedPlan is the on-the-wire shape agent emits to the executor.
// Distinct from config.RunConfig because its Version and Vars fields
// carry the omitempty tag — without it, an empty agent plan would
