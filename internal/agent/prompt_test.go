...
func TestBuildSystemPrompt_StyleStep_Snapshot(t *testing.T) {
	got, err := buildSystemPrompt(StyleStep, nil, nil)
	if err != nil {
		t.Fatalf("buildSystemPrompt: %v", err)
	}
...
// switch default: a stray env var or agent.yml typo should still
...
// agent-feedback-non-cmd fix: without rendering, summarizeStep'd lines
