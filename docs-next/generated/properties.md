...
|----------|------|----------|-------------|
| `capture_content` | boolean | No | Capture full file content before/after (default: false) |
| `embed_plan` | boolean | No | Embed full plan in artifact for LLM context (default: true if <= max_plan_steps) |
| `format` | string | No | Output format (json, markdown, or both) (allowed: `json, markdown, both`) |
| `include_checksums` | boolean | No | Include SHA256 checksums (default: true) |
| `max_diff_size` | integer | No | Maximum diff size in bytes per file (default: 1MB) |
| `max_plan_steps` | integer | No | Don't embed plan if exceeds this many steps (default: 20) |
...
| `max_lines_changed` | integer | No | Maximum total lines changed (added + removed) |
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `artifact_capture` | any | No | Capture file changes with enhanced metadata for LLM agents |
| `artifact_validate` | any | No | Validate artifacts against constraints (change budgets) |
...
| `repo_apply_patchset` | any | No | Apply multiple patches to multiple files atomically |
