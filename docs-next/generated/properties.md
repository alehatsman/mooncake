...
<!-- Version: dev | Generated: 2026-05-17 13:13:56 CEST -->
...
| `pkg.hold` | any | No | Mark or unmark packages as held to prevent upgrade/removal (apt on linux, brew on darwin) |
| `pkg.list` | any | No | Return the installed packages and versions (read-only; apt on linux, brew on darwin) |
| `pkg.repo` | any | No | Manage a third-party package repository (apt + brew taps; dnf deferred) |
| `pkg.upgrade` | any | No | Upgrade named packages or all installed packages (apt on linux, brew on darwin) |
