...
<!-- Version: dev | Generated: 2026-05-17 14:20:44 CEST -->
...
| `pkg.list` | any | No | Return the installed packages and versions (read-only; apt + dnf on linux, brew on darwin) |
| `pkg.repo` | any | No | Manage a third-party package repository (apt, dnf/yum, brew taps) |
| `pkg.upgrade` | any | No | Upgrade named packages or all installed packages (apt + dnf on linux, brew on darwin) |
